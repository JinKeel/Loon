        ...
        obj.data.forEach(item => {
            if (item.extraDataArr) {
                for (let k in item.extraDataArr) {
                    ...
                    if (/reportExpose|reportProgress/i.test(k)) {
                        item.extraDataArr[k] = "";
                    }
                    if (/APP_ID|APP_KEY/i.test(k)) {
                        item.extraDataArr[k] = "0";
                    }
                    if (/PushHost|PostHost|Api\.Host/i.test(k)) {
                        item.extraDataArr[k] = "";
                    }
                    if (/MediaPlayer/i.test(k)) {
                        item.extraDataArr[k] = "";
                    }
                    if (/useDDI|DDIEvent|SessionId/i.test(k)) {
                        item.extraDataArr[k] = "0";
                    }
